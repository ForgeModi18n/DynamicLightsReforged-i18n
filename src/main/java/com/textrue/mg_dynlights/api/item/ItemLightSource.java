...
package com.textrue.mg_dynlights.api.item;
...
public class ItemLightSource
{
    ...
    public static @NotNull Optional<ItemLightSource> fromJson(@NotNull ResourceLocation id, @NotNull JsonObject json)
    {
        if (!json.has("item") || !json.has("luminance")) {
            MgDynamicLights.warn("Failed to parse item light source \"" + id + "\", invalid format: missing required fields.");
        ...
        if (luminanceElement.isNumber()) {
            ...
            MgDynamicLights.warn("Failed to parse item light source \"" + id + "\", invalid format: \"luminance\" field value isn't string or integer.");
            return Optional.empty();
        }
